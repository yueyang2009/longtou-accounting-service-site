      ...
      <main>
        ...
        <section className="border-b border-brand-line">
          <div className="mx-auto max-w-3xl px-6 py-24 text-center md:py-32">
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.12em] text-brand-muted">团队</p>
            ...
            <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
              {trustMetrics.map((metric) => (
                <div key={metric.label} className="text-center">
                  <p className="text-3xl font-semibold text-brand-emerald md:text-4xl">{metric.value}</p>
                  <p className="mt-2 text-sm text-brand-muted">{metric.label}</p>
                  {metric.detail ? (
                    <p className="mt-1 text-xs text-brand-muted/70">{metric.detail}</p>
                  ) : null}
                </div>
              ))}
            </div>
        ...
        <section className="border-b border-brand-line">
          <div className="mx-auto max-w-7xl px-6 py-24 md:py-28">
            <div className="mx-auto max-w-3xl text-center">
              <p className="mb-4 text-xs font-semibold uppercase tracking-widest text-brand-muted">核心顾问</p>
              <h2 className="text-3xl font-semibold leading-tight tracking-tight text-brand-ink md:text-4xl">
                先看人，再谈合作
              </h2>
              <p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-brand-muted">
        ...
        <section className="border-b border-brand-line bg-brand-soft">
          <div className="mx-auto max-w-7xl px-6 py-24 md:py-28">
            <div className="mx-auto max-w-3xl text-center">
              <p className="mb-4 text-xs font-semibold uppercase tracking-widest text-brand-muted">行业顾问组</p>
              <h2 className="text-3xl font-semibold leading-tight tracking-tight text-brand-ink md:text-4xl">
                按行业配置专属顾问组
              </h2>
              <p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-brand-muted">
            ...
            <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {extendedExperts.map((group) => (
                <div
                  key={group}
                  className="flex items-center gap-3 rounded-card border border-brand-line bg-white px-6 py-5"
                >
                  <span className="h-2 w-2 shrink-0 rounded-full bg-brand-gold" />
                  <p className="text-sm font-medium text-brand-ink">{group}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="border-b border-brand-line">
          <div className="mx-auto max-w-7xl px-6 py-24 md:py-28">
            <div className="mx-auto max-w-3xl text-center">
              <p className="mb-4 text-xs font-semibold uppercase tracking-widest text-brand-muted">团队理念</p>
              <h2 className="text-3xl font-semibold leading-tight tracking-tight text-brand-ink md:text-4xl">
                我们坚持什么
              </h2>
            </div>
            <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {teamValues.map((value) => (
                <div key={value.title} className="rounded-card border border-brand-line bg-white p-6">
                  <value.icon className="h-8 w-8 text-brand-emerald" />
        ...
        <section className="border-b border-brand-line bg-brand-soft">
          <div className="mx-auto max-w-3xl px-6 py-24 text-center md:py-28">
            ...
            <div className="mt-10 flex flex-col items-center gap-4">
              ...
              <p className="text-sm text-brand-muted">每月仅服务有限企业，需初步评估适配性</p>
            </div>
          </div>
        </section>
      </main>
